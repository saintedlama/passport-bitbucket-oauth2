...
function Strategy(options, verify) {
  ...
  options.skipEmails = !!options.skipEmails;
  ...
  if (!options.customHeaders['User-Agent']) {
    ...
    options.customHeaders.Authorization = 'Basic ' + Buffer.from(options.clientID + ':' + options.clientSecret).toString('base64');
  }
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body) {
    ...
    if (self.options.skipEmails) {
      return done(null, profile);
    }

    self._oauth2.get(self._userEmailsURL, accessToken, function (err, body) {
      try {
        json = JSON.parse(body);
      } catch (ex) {
      ...
      profile._rawEmails = body;
      profile._jsonEmails = json;
